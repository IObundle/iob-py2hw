...
class iob_or(iob_module):
    ...
    def __init__(self, name, port_matrix, param_matrix = []):
        '''Constructor for the OR gate module'''
        self.name = name
        self.port_list = []
...
def unit_test():
    ...
    # Check if iob_or instance is correct
    assert or0.name == 'or0'
    assert or0.port_list[0].name == 'i0'
    assert or0.port_list[0].width == 2
    assert or0.port_list[0].value == wire0
    assert or0.port_list[0].direction == 'input'

    assert or0.port_list[1].name == 'o0'
    assert or0.port_list[1].width == 1
    assert or0.port_list[1].value == wire2
    assert or0.port_list[1].direction == 'output'
